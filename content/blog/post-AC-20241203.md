...
3.1
Hasuraと Next.js の環境が整ったので、最後にCode Generatorで自動生成できるように設定します。
...
4.2
上記のように修正したら`localhost:3000` にアクセスして動作確認していきます。
...
## 余談
BINGOプロダクトを初めて2年が経ちますがcode geneaterを導入したのは今年からでした。せっかくなので導入前と後の比較を行い、自動生成は使うべきという教訓を皆様に共有します。
